...
Проблема:
    1. lime скачивает PKGBUILD и анализирует его
    2. Злоумышленник меняет PKGBUILD в AUR
    3. paru/yay скачивает PKGBUILD ЗАНОВО — уже вредоносный
    4. Устанавливается скомпрометированная версия

Почему "проверить хэш дважды" НЕ решает проблему:
    Между второй проверкой хэша и реальной установкой paru всё равно
    есть временное окно. Кроме того, paru сам скачивает PKGBUILD
    независимо от нашей проверки — мы не контролируем что он скачает.

Правильное решение:
    Сохранить проверенный PKGBUILD на диск и передать его в makepkg
    напрямую через флаг --pkgbuild (если поддерживается хелпером),
    либо собрать пакет самостоятельно через makepkg в подготовленной
    директории — тогда paru/yay используется только для установки
    уже собранного .pkg.tar.zst.

Текущая реализация:
    Сохраняем проверенный PKGBUILD в /tmp/lime-verified/<pkg>/PKGBUILD,
    меняем рабочую директорию и вызываем makepkg напрямую.
    paru/yay используется только для разрешения зависимостей если нужно.
